...
Make a VAE train loaded which only contains Mal/Ben for the retrainining (does this matter - ask sam)
'''
...
mlp_save_results_path = r"VAE_lung_lesion/VAE_RandomSearch/MLP.pt"
retrained_results_path = r"VAE_lung_lesion/VAE_RandomSearch/retrained_VAE_params.pt"
...
def loss_function(recon_x, x, mu, logvar, epoch, hyperparams, retrain_indicator, labels):
    ...
    beta_vae_loss = recon_mix + beta_norm*kld*annealing_weight
...
# parameter_space = {"HU_UpperBound":[500],      # Upper bound of Hounsfield units
#                    "HU_LowerBound":[-1000], # Lower bound of Hounsfield units
#                    "base":[32],   #[24, 32, 64, 128],             # number of feature maps in convolutional layers
#                    "latent_size": [16], # base*latent_size = size of latent space
#                    "annealing":[1],                 # annealing on KL Divergence indicator (0 for NO or 1 for YES)
#                    "ssim_indicator":[2],          # SSIM indicator for loss function: 0 to not inlcude, 1 to include, 2 to use MS-SSIM
#                    "batch_size":[256],    # batch size (bigger for more stable training)
#                    "alpha":[0.85],            # If using SSIM in loss function this is the balance between reconstruction loss (L1/MAE) and the other metric in alpha*L1_loss + (1-alpha)*other_metric
#                    "beta":[5], # multiplier for KL divergence, helps to disentangle latent space. Idea from beta-VAE: https://openreview.net/pdf?id=Sy2fzU9gl
#                    "lr":[5e-4],    # learning rate (smaller number for slower training)
...
train_start = perf_counter()
b, Mal_Ben, b2, Mal_Ben2 = test_hyperparams(parameter_space, data=2, vae_test_loss=vae_test_loss, meta_location=meta_location, labels_path=labels_path, num_runs=15, nepochs=150) #12
train_stop= perf_counter()
...
def retrain_VAE_with_MLP(vae_hyperparams, mlp_hyperparams, data, mlp_retrain_train_loader, mlp_retrain_valid_loader, mlp_retrain_test_loader):
    global latent_size, base, mlp_save_results_path, vae_train_loader, vae_test_loader, vae_model, mlp_model, batch_size, epochs, retrained_results_path
    if data == 1:
        # malignant vs non-malignant
        meta = pd.read_csv(meta_location + '//' + 'meta_mal_nonmal.csv')
        latent_vectors = np.load(results_path + '/' + "latent_vectors_{}.npy".format(Run), allow_pickle=True)
    ...
    #r"/nobackup/mm17b2k/VAE_RandomSearch/retrained_VAE_params.pt"
...
retrain_VAE_with_MLP(vae_hyperparams, best_MLP_hyperparams_mal_ben, 2, mlp_retrain_train_loader, mlp_retrain_valid_loader, mlp_retrain_test_loader)
train_stop= perf_counter()
VAE_MLP_training_time = train_stop - train_start
print('M v B VAE with Classifier Loss Training Time', VAE_MLP_training_time)

vae_model.load_state_dict(torch.load(retrained_results_path, map_location=torch.device('cpu'))["state_dict"])
...
latent_vectors2 = get_mal_ben_latent_vecs() 
b, Mal_Ben_retrained, b2, Mal_Ben2_retrained = test_hyperparams(best_MLP_hyperparams_mal_ben_copy, data=2, vae_test_loss=vae_test_loss, meta_location=meta_location, labels_path=labels_path, num_runs=1, nepochs=200) #20
train_stop= perf_counter()
MLP4_training_time = train_stop - train_start
print("Malignant vs Benign Re-trained MLP Training Time:", MLP4_training_time)

train_start = perf_counter()
b, Mal_Ben_retrained2, b2, Mal_Ben2_retrained2 = test_hyperparams(parameter_space, data=2, vae_test_loss=vae_test_loss, meta_location=meta_location, labels_path=labels_path, num_runs=5, nepochs=150)
train_stop= perf_counter()
MLP4_training_time = train_stop - train_start
print("2nd Search: Malignant vs Benign Re-trained MLP Training Time:", MLP4_training_time)
...
print("malignant vs benign re-trained 2 AUC",
      'AUC:', Mal_Ben_retrained2[0], 'Test Loss:', Mal_Ben_retrained2[1], 'Test Accuracy:', Mal_Ben_retrained2[2],
      'Performance Metrics:', Mal_Ben_retrained2[3], "Hyperparams", Mal_Ben_retrained2[4])
