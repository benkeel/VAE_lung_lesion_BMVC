...
save_results_path = r"/nobackup/mm17b2k/Dirichlet_VAE_RandomSearch"
...
class LoadData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        y = self._y[index, :]
        return x, y
latent_vectors = np.load(r"/nobackup/mm17b2k/Dirichlet_VAE_RandomSearch/latent_vectors_{}.npy".format(candidate_num))
...
def test_hyperparams(parameter_space, data):
    ...
    if data == 1:
        ...
        latent_vectors = latent_vectors = np.load(r"/nobackup/mm17b2k/Dirichlet_VAE_RandomSearch/latent_vectors_{}.npy".format(candidate_num))
    ...
    while len(hyperparams_list) < 55:
        ...
        for run in range(5):  # 5-fold cross validation
            ...
            loss, accuracy, results, auc = train_model(350, train_loader, valid_loader, test_loader, hyperparams)
            if accuracy < 0.60:
        ...
        if stop_counter == 100:
            ...
            accuracy_list = np.nan_to_num(accuarcy_list).tolist() 
    ...
    if len(auc_list) > 1:
        print("Based on AUC:",
            ...
            'Performance Metrics:', results_list[idx],
            'Index:', idx,
            'Hyperparameters:', hyperparams_list[idx],
            'Maximum and Minimum AUC and Accuracy:', best_results[idx])
    accuracy_list = np.nan_to_num(accuracy_list).tolist() 
    idx = accuracy_list.index(max(accuracy_list))
...
# parameter_space = {"threshold":[0.6,0.55,0.5,0.45,0.4], "lr":[1e-6,5e-5,1e-5], 
#                    "layer_sizes":[[4096, 4096, 2048], [4096, 4096, 1024], [4096, 2048, 1024], [4096, 1024, 1024],
...
#                                   [512, 256, 256]],
#                    "dropout":[0.4,0.5,0.55], "batch_size":[32,64,128,256,512],
#                    "Depth":[4,5]
#                   }
parameter_space = {"threshold":[0.6,0.55,0.5,0.45,0.4], "lr":[1e-6,1e-5,5e-5],
                   "layer_sizes":[[2048, 2048, 1024], [2048, 1024, 512], [2048, 1024, 256], [2048, 512, 512],
                                  ...
                                  [1024, 512, 512], [1024, 512, 256], [1024, 256, 256], [512, 512, 256],
                                  [512, 256, 256]],
                   "dropout":[0.2,0.25,0.4,0.5,0.6], "batch_size":[32,64,128,256,512],
                   "Depth":[4, 5]
                   }
...
accuracy_list = np.nan_to_num(accuracy_list).tolist()
idx = accuracy_list.index(max(accuracy_list))
if idx != 0 or len(accuracy_list) > 1:
    print("Based on Accuracy",
      "malignant vs non-malignant",
      'AUC:', auc_list[idx], 'Test Loss:', loss_list[idx], 'Test Accuracy:', accuracy_list[idx], 
      'Performance Metrics:', results_list[idx],'Index:', idx, 
