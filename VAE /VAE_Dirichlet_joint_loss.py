...
retrained_results_path = r"/nobackup/mm17b2k/Dirichlet_VAE_RandomSearch/retrained_VAE_params.pt"
meta_location = r"VAE_lung_lesion/Preprocessing"
labels_path = r"VAE_lung_lesion/MLP/latent_vectors"
mlp_save_results_path = r"/nobackup/mm17b2k/Dirichlet_VAE_RandomSearch/MLP.pt"
...
class DIR_VAE(nn.Module):
   ...
   # Reconstruction + KL divergence losses summed over all elements and batch
    def loss_function(self, recon_x, x, mu, epoch, hyperparams, retrain_indicator, labels):
        '''
        ...
        # increase the effect of KL divergence over time
        ...
        kld = resampler.prior_forward(mu)
        ...
        #beta_norm = (beta*latent_size*base)/(64*64)   #(10*beta*latent_size)/(64*64*batch_size)   (maybe change back to thsi?)
        beta_norm = (10*beta*latent_size)/(64*64*batch_size)


        beta_vae_loss = recon_mix + beta_norm*(kld - C).abs() # beta_norm*kld*annealing_weight
...
def train(epoch, optimiser, sample_shape, train_loader, vae_model, hyperparams, retrain_indicator):
    ...
    for batch_idx, dataset in enumerate(train_loader):
        ...
        recon_batch, alphas, dirichlet_sample = vae_model(data) # Feed one batch through VAE model

        # Calculate loss and save various metrics
        loss, recon_loss, kld, ssim_score, pure_loss, mlp_loss = vae_model.loss_function(recon_batch, data, alphas, epoch, hyperparams, retrain_indicator, labels)
...
def test(epoch, test_loader, vae_model, hyperparams, retrain_indicator):
    ...
    with torch.no_grad():
        for i, dataset in enumerate(test_loader):
            ...
            recon_batch, alphas, dirichlet_sample = vae_model(data)
            # Calculate loss and save various metrics
            testloss, recon_loss, kld, ssim_score, pure_loss, mlp_loss = vae_model.loss_function(recon_batch, data, alphas, epoch, hyperparams, retrain_indicator, labels)
            # Update running losses
...
parameter_space = {"HU_UpperBound":[500],      # Upper bound of Hounsfield units
                   ...
                   "base":[64],   #[24, 32, 64, 128],             # number of feature maps in convolutional layers
                   "latent_size": [8], # base*latent_size = size of latent space
                   ...
                   "batch_size":[128],    # batch size (bigger for more stable training)
                   ...
                   "beta":[2], # multiplier for KL divergence, helps to disentangle latent space. Idea from beta-VAE: https://openreview.net/pdf?id=Sy2fzU9gl
                   "lr":[5e-5],    # learning rate (smaller number for slower training)
                   "ssim_scalar":[1],                 # multiplier for SSIM: 1 for no upweighting, 2 for multiply by batch_size
                   "recon_scale_factor":[1],     # multiplier for reconstrction loss
                   "alpha_fill_value":[0.95]
                   }
...
epochs = 400
vae_model = DIR_VAE(hyperparams)
vae_model = vae_model.to(device)
print('parameter count:', parameter_count(DIR_VAE(hyperparams)))
train_start = perf_counter()
vae_train_loader, vae_test_loader  = vae_data_split(batch_size, hyperparams, all_files_list, meta_location)
test_loss, ssim_score = train_VAE_model(vae_model, epochs = epochs, sample_shape = (12, latent_size*base), Run=Run, train_loader=vae_train_loader, test_loader=vae_test_loader, hyperparams=hyperparams, retrain_indicator=0)
...
def retrain_VAE_with_MLP(vae_hyperparams, mlp_hyperparams, data, mlp_retrain_train_loader, mlp_retrain_valid_loader, mlp_retrain_test_loader):
    ...
    vae_model = DIR_VAE(vae_hyperparams)
    print('VAE parameter count:', parameter_count(DIR_VAE(hyperparams)))
    vae_model = vae_model.to(device)
    vae_model.load_state_dict(torch.load(pre_trained_vae_results_path, map_location=torch.device('cpu'))["state_dict"])
    vae_model.train()
    epochs = 150
    test_loss, ssim_score = train_VAE_model(model = vae_model, epochs = epochs, sample_shape = (12, latent_size*base), Run=Run, train_loader=vae_train_loader, test_loader=vae_test_loader, hyperparams=vae_hyperparams, retrain_indicator=1)
