...
def train(epoch, optimiser, sample_shape, train_loader):
    global vae_model
    vae_model.train()
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        loss, recon_loss, kld, ssim_score, pure_loss = loss_function(recon_batch, data, mu, logvar, epoch)
...
def test(epoch, test_loader):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            data = data.float().to(device)
            recon_batch, mu, logvar = vae_model(data)
            testloss, recon_loss, kld, ssim_score, pure_loss = loss_function(recon_batch, data, mu, logvar, epoch)
...
def train_model(model, lr, epochs, sample_shape, Run, train_loader, test_loader):
    ...
    for epoch in range(1, epochs + 1):
        train_loss, ssim_score, kld = train(epoch, optimiser, sample_shape, train_loader)
        test_loss, test_ssim = test(epoch, test_loader)
...
parameter_space = {"HU_UpperBound":[300, 400, 500, 600], "HU_LowerBound":[-1000, -800, -700, -500],
                    ...
                    "batch_size":[64, 128, 256, 512], "ssim_scalar":[1,2]
                  }      
...
test_loss, ssim_score = train_model(vae_model, lr=lr, epochs = epochs, sample_shape = (12, latent_size*base, 1, 1), Run=Run, train_loader=train_loader, test_loader=test_loader) 
...
def test_hyperparams(parameter_space, data, vae_test_loss):
    ...
    if not math.isnan(vae_test_loss):
        ...
        if len(auc_list) > 1:
            auc = np.nan_to_num(auc_list).tolist()
            idx = auc_list.index(max(auc_list))
            print("Based on AUC:",
              'AUC:', max(auc_list), 
              'Test Loss:', loss_list[idx], 'Test Accuracy:', accuracy_list[idx], 
